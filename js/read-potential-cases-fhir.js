...
(async () => {
  ...
  await fs.writeFile("glomerulonephritis-potential-cases.csv", "patient-id,dob,codes,last-encounter\n");
  for(let patient in patients) {
    try {
      ...
      await fs.appendFile("glomerulonephritis-potential-cases.csv", row);
    } catch(error) {
      console.log(error);
    }
  }
